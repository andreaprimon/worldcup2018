		...
		try {
			...
			if (array != null && array.length() > 0) {
				for (int i = 0; i < array.length(); i++) {
					...
					item.setFullNameTeam1(getStringValue(obj,
							"match_host_full_name"));
					item.setFullNameTeam2(getStringValue(obj,
		...
		try {
			...
			if (array != null && array.length() > 0) {
				for (int i = 0; i < array.length(); i++) {
					obj = array.getJSONObject(i);
					item = new Match();
					country1 = getCountry(getStringValue(obj, "host_name_full"));
					country2 = getCountry(getStringValue(obj,
							"guests_name_full"));
					item.setMatchId(getIntValue(obj, "id"));
					item.setFullNameTeam1(country1.getCountryCode());
					item.setWinTeam(getCountry(
		...
		try {
			...
			if (array != null && array.length() > 0) {
				for (int i = 0; i < array.length(); i++) {
					...
					item.setCountryName(getStringValue(obj, "team_full_name"));
