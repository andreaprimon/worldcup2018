...
package com.projectemplate.worldcup2014.database;
...
import com.projectemplate.worldcup2014.database.binder.CountryInfoBinder;
import com.projectemplate.worldcup2014.database.binder.JsonOfflineBinder;
import com.projectemplate.worldcup2014.database.binder.MatchInfoBinder;
import com.projectemplate.worldcup2014.database.mapper.CountryInfoMapper;
import com.projectemplate.worldcup2014.database.mapper.GroupInfoMapper;
import com.projectemplate.worldcup2014.database.mapper.JsonofflineMapper;
import com.projectemplate.worldcup2014.database.mapper.MatchInfoMapper;
import com.projectemplate.worldcup2014.database.mapper.StadiumMapper;
...
public final class DatabaseUtility {
	...
	private static String STRING_SQL_INSERT_INTO_JSON_OFFLINE = "INSERT OR IGNORE INTO JsonOffline(link,content)VALUES(?,?)";
	...
	public static boolean insertJson(Context context, JsonOffline matchInfo) {
		PrepareStatement statement = new PrepareStatement(context);
		return statement.insert(STRING_SQL_INSERT_INTO_JSON_OFFLINE, matchInfo,
				new JsonOfflineBinder());
	}

	// search
	
	public static boolean checkExistsJson(Context context, String jsonLink) {
		PrepareStatement statement = new PrepareStatement(context);
		ArrayList<JsonOffline> list = statement.select(DBKeyConfig.TABLE_JSON,
				"*", DBKeyConfig.KEY_JSON_LINK + "='" + jsonLink + "'",
				new JsonofflineMapper());
		
		return list.size()>0;
	}

	public static String getJsonContent(Context context, String jsonLink) {
		PrepareStatement statement = new PrepareStatement(context);
		ArrayList<JsonOffline> list = statement.select(DBKeyConfig.TABLE_JSON,
				"*", DBKeyConfig.KEY_JSON_LINK + "='" + jsonLink + "'",
				new JsonofflineMapper());
